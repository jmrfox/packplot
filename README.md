...
- `solver="pymoo"`: reserved stub for future pymoo-based optimization (currently raises `NotImplementedError`).

Supported source formats currently include raster images (`.png`, `.jpg`, `.jpeg`) and `.svg` inputs.
SVG loading uses a dedicated loader path and requires `cairosvg` at runtime.
...
- `optimize_config.compact_layout.method`: `"lbfgsb"`, `"de"` (differential evolution), or `"hybrid"`.
...
print(result.minimum_clearance, result.outside_violation_magnitude)
print(result.solver_method, result.solver_iterations, result.solver_success)
