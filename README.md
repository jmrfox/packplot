...
- `solver="circpacker"`: maps objects to circle slots generated by the `circpacker` package.
- `solver="cell_packing"`: polygon relaxation dynamics using `cell-packing` (`polypacker`).
...
- `circpacker` works by fitting each object into a circle and assigning generated circle slots.
- `cell_packing` currently requires a local checkout path (default: `third_party/cell-packing`) because the upstream wheel packaging is incomplete.
...
result = pack_images(
    ...
    options=PackOptions(
        ...
        optimize_config=OptimizeConfig(
            ...
            spread_objective=SpreadObjectiveConfig(
...
Legacy flat optimizer arguments are still accepted for backward compatibility, but new code should prefer `optimize_config` with explicit phase/method blocks.